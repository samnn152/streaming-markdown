...
The Flutter 3.10 CI job resolves the root package and example with
`flutter pub get`. The example Android host uses the standard Gradle
Groovy template with v2 embedding metadata, which Flutter 3.10 and modern
Flutter releases can identify cleanly. The same legacy job builds the
example for Web after the package analyze and test gates, so Dart/widget
