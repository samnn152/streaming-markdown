...
Keep the parser and selection controller in `State`, not in `build`. For a chat
list, give each message a stable key and keep the message renderer alive while
it is offscreen; this preserves settled animation, selection, and controller
state when a new response is appended.
...
The selection engine uses the same source snapshot while chunks arrive. Users
can select formatted text, tables, code, images, LaTeX, and HTML blocks; the
highlight remains one flat visual selection and edge dragging scrolls like a
`TextField`. Rich copy is available on Web and all native targets, with plain
text fallback when the clipboard host cannot accept HTML.
                ...
                slivers: [
                  SliverPadding(
                    ...
                    sliver: AnimatedStreamingMarkdown(
                      ...
                      placeholder: 'Tap Start to stream Markdown.',
...
`CustomScrollView`. When selection is enabled, the explicit selection area
keeps source ranges stable while sliver children mount and unmount.
...
- Use `AnimatedStreamingMarkdownSelectionArea` for selectable slivers.
- Dispose app-owned parser and selection controllers from `dispose`.
